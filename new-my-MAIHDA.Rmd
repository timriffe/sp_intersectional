...
收入变量 C10_1_1_open
C10_1_1_open is a continuous variable
First define 9999998 and 9999999 as missing values
Then group the "C10_1_1_open" to get new variable "income"
0: 80~2300: Low income
1: 2301/5000: middle income
2: 5001/10000: upper-middle income
3: Others: high income
```{r}
table(df$C10_1_1_open)
df$C10_1_1_open[df$C10_1_1_open %in% c(9999998, 9999999)] <- NA
df$income <- cut(df$C10_1_1_open, breaks = quantile(df$C10_1_1_open, probs = seq(0, 1, by = 1/3), na.rm = TRUE), labels = c("Low", "Medium", "High"), include.lowest = TRUE)
levels(df$income) <- c("Low Income", "Medium Income", "High Income")
table(df$income)
```
More than half of the values "income" are missing
Linear imputation of income based on household expenditure variables"C11_1_1_open" ("family expense", fewer missing values)
Get a continuous variable with a value of 1~3
In order to ensure that the categories are positive, the imputed continuous variables are classified (rounded)
(1~1.5 = 1) (1.5~2.5 = 2) (2.5~3 = 3)
This step is done in stata, I don’t know how to implement it in R
```{r}
# 对income进行线性插补
# Perform linear interpolation on income
table(df$C11_1_1_open)
# 重新编码C11_1_1_open中的缺失值 
# Recode missing values in C11_1_1_open
df$C11_1_1_open[df$C11_1_1_open %in% c(999998, 999999)] <- NA

# 使用线性插补填充income 
# Use linear interpolation to fill income
df$a <- approx(df$C11_1_1_open, df$income, method = "linear", rule = 2)$y

# 调整incomenew为正整数 
# Adjust incomenew to a positive integer
df$incomenew <- cut(df$a, breaks = c(1, 1.5, 2.5, 3), labels = c(1, 2, 3), include.lowest = TRUE)

# 移除中间变量a 
# Remove intermediate variable a
...
Remove a small number of missing values
```{r}
Data <- Data[!is.na(Data$eduu), ]

Data <- Data[!is.na(Data$srhh), ]
Data <- Data[!is.na(Data$sp), ]
Data <- Data[!is.na(Data$cedd), ]
```
